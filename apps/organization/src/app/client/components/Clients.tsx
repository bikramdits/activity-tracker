...
import { allClientsData } from '../../types/clientsTypes';
...
const Clients = () => {
  ...
  const { data, refetch } = useClients({
    ...
    filterBy:  selectedProject?.length || selectedDepart?.length ? id : '',
  ...
  const updateByClientArchive = async () => {
    const updatedValue = {
      isArchive: true,
    };
    try {
  ...
  const { mutateAsync: mutateAsyncOne } = useClientArchive();
  ...
  return (
    <div className="bg-gray-50 w-full pb-2 mt-16 overflow-hidden">
      ...
      <div className="headerdiv flex justify-between p-3 mt-2 -mb-2">
        ...
        <div className="allbtn flex gap-4">
          <ButtonWithIcon
            icon={
              <AddIcon
              sx={{
      ...
      <div className="m-3 border border-slate-300 rounded-lg">
        <div className="flex justify-between items-center rounded-t-[7px]  flex-wrap overflow-auto border-b-[1px] bg-white">
          {/* leftdiv dropdwon */}
      ...
      {isArchiveModalOpen && (
        <DeleteModal>
          ...
          <DeleteConfirmModal
            close={closeModal}
            onConfirm={updateByClientArchive}
            deleteIcon={deleteImg}
            message="Do you really want to archive this client?"
